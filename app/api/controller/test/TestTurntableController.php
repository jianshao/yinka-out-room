...
use app\domain\game\turntable\baolv\BaolvTaskState;
...
class TestTurntableController extends BaseController
{
    public function baolvTaskInfo() {
        $taskId = $this->request->param('taskId');
        $task = TurntableBaolvService::getInstance()->loadTask($taskId);
        if ($task == null) {
        ...
        if ($task->state == BaolvTaskState::$FINISH) {
            $taskId = $task->taskId;
            return header("Location:http://recodetest1.fqparty.com/static/tasks/$taskId.csv");
        }
    ...
    public function publishBaolvTask() {
        ...
        $box = TurntableSystem::getInstance()->findBox($boxId);
        if ($box == null) {
            return rjson([], 500, 'boxId参数错误');
        }
        ...
        $taskId = TurntableBaolvService::getInstance()->publishTask($boxId, $userCount, $loopCount, $breakCountPerLoop, $isUserBreakCount, $isSync);

        return header("Location:http://recodetest1.fqparty.com/api/test/turntable/baoLvTaskInfo?taskId=$taskId");
    }
    ...
    public function refreshPool() {
        ...
        $runningRewardPool = TurntableService::getInstance()->refreshRewardPool($boxId, $poolId);
    ...
    public function getBox2User() {
        ...
        $box2User = TurntableUserDao::getInstance()->loadBoxUser($userId, $boxId);
    ...
    public function clearBox2User() {
        ...
        TurntableUserDao::getInstance()->removeBoxUser($userId, $boxId);
        return rjson([]);
    }
    ...
    public function getBox2RunningPool() {
        ...
        $runningRewardPool = TurntableService::getInstance()->loadRunningRewardPool($boxId, $poolId);

        return rjson($this->encodeRunningRewardPool($runningRewardPool, 1));
    }
