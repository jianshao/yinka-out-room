...
class UploadOssFileCommon
{
    ...
    public function ossFileUpload($file_name,$path=1)
    {
        // if (is_file(__DIR__ . '/../autoload.php')) {
        //     require_once __DIR__ . '/../autoload.php';
        // }
        ...
        try {
            $ossClient = new OssClient($accessKeyId, $accessKeySecret, $endpoint);
            $result = $ossClient->uploadFile($bucket, $file_name, $file_name);//上传成功
            return $result['info']['url'];
        } catch (OssException $e) {
            printf(__FUNCTION__ . ": FAILED\n");
            printf($e->getMessage() . "\n");
     ...
     */
    public function delossFile($objects)
    {
        if (is_file(__DIR__ . '/../autoload.php')) {
            require_once __DIR__ . '/../autoload.php';
        }
        ...
        $ossClient = new OssClient($accessKeyId, $accessKeySecret, $endpoint);
        try{
            $delobject = $ossClient->deleteObjects($bucket, $objects);
            if($delobject){
                return true;
            }
        } catch(OssException $e) {
