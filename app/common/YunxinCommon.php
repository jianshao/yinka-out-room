...
class YunxinCommon
{
	...
	public function json_to_array($json_str)
	{
        ...
        $json_arr = array();
        if (!is_array($json_str) || empty($json_str)) {
            return $json_arr;
        }
	...
	public function updateUserToken($accid)
	{
        ...
        Log::info(sprintf('YunxinCommon::updateUserToken $result=%s', json_encode($result)));
		return $result;
	}
	 ...
	 * @param  $accid [云信ID，最大长度32字节，必须保证一个APP内唯一（只允许字母、数字、半角下划线_、@、半角点以及半角-组成，不区分大小写，会统一小写处理）]
	 * @param  $faccid [要修改朋友的accid]
     * @param  $isDeleteAlias [是否需要删除备注信息默认false:不需要，true:需要]
	 * @return $result      [返回array数组对象]
	 */
	public function deleteFriend($accid, $faccid, $isDeleteAlias = false)
	{
    ...
    /**
     * 消息功能-批量给多个用户发送单聊消息。  (最多可批量给 500 个账号发送单聊消息)
     * 该 API 的调用存在频控，单个应用一分钟最多调用 120 次，超过则返回 416 状态码，并且将被屏蔽一段时间。
     * @param $fromAccid
     ...
     * @return array
     */
    public function sendBatchMsg($fromAccid, $toAccids, $type, $body)
    {
