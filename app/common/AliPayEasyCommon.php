...
class AliPayEasyCommon
{
    ...
    public function init($outer_order_no, $certName, $certNo, $channel, $biz_code, $config)
    {
        ...
        if($channel == 'appStore') {
            $merchantConfig->returnUrl = $config == 'muaconfig' ? 'likeaiyu://com.ali.verify'  : "likeaiyu://com.ali.verify";   //ios回跳地址
        } else {
    ...
    function getOptions()
    {
        ...
        $options->appId = config('config.alipay_yuansheng1.app_id');   //请填写您的AppId，例如：2019022663440152

        // 为避免私钥随源码泄露，推荐从文件中读取私钥字符串而不是写入源码中
        $options->merchantPrivateKey = config('config.alipay_yuansheng1.private_key');   //请填写您的应用私钥，例如：MIIEvQIBADANB ... ...
...
//        $options->merchantCertPath = '<-- 请填写您的应用公钥证书文件路径，例如：/foo/appCertPublicKey_2019051064521003.crt -->';
        ...
        //注：如果采用非证书模式，则无需赋值上面的三个证书路径，改为赋值如下的支付宝公钥字符串即可
         $options->alipayPublicKey = config('config.alipay_yuansheng1.ali_public_key'); //请填写您的支付宝公钥，例如：MIIBIjANBg...
