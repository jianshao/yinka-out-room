...
class GetuiCommon
{
    ...
    private function init()
    {
        ...
        if (isset($conf['getui'])) {
            $this->appid = config($source . 'config.getui.appid');
            $this->appkey = config($source . 'config.getui.appkey');
            $this->mastersecret = config($source . 'config.getui.mastersecret');
            $this->host = config($source . 'config.getui.host');
        } else {
    ...
    //单推接口
    function pushMessageToSingle($cid, $type)
    {
        $igt = new \IGeTui($this->host, $this->appkey, $this->mastersecret);
//        $template = $this->IGtNotificationTemplateDemo('123', '123', '123', '123'); // 通知栏消息模板
        $template = $this->IGtTransmissionTemplate('', '', '', $type);  // 穿透消息模板
        //定义"SingleMessage"
     ...
     * string(2) "ok"
     * ["contentId"]=>
     * string(30) "OSL-1230_hQRdwhGpYMAO4d2eHrnSV"
     * }
     */
    public function pushMessageToList($cid, $type = 0, $payload = '', $title = "", $content = "")
    {
        $igt = new \IGeTui($this->host, $this->appkey, $this->mastersecret);
        $template = $this->IGtTransmissionTemplate($title, $content, $payload, $type);   //透传消息模板
        //定义"ListMessage"信息体
        ...
        foreach ($cid as $k => $v) {
            ...
            $tmp = (string)$v;
            $target->set_alias($tmp);     //设置别名
            $targetList[] = $target;
        }
        Log::record("个推日志aaaaaaaaa-----" . json_encode($targetList), "info");
     ...
     * string(2) "ok"
     * ["contentId"]=>
     * string(31) "OSL-1230_32UmSPUcUV9wR7tOlXG8h6"
     * }
     */
    public function pushMessageToNotification($cid, $type = 0, $payload = '', $title = "", $content = "")
    {
        if (config("config.appDev")==='dev'){
            return [
                'result'=>'ok',
                'contentId'=>"OSL-1230_32UmSPUcUV9wR7tOlXG8h6"
            ];
        }
        $igt = new \IGeTui($this->host, $this->appkey, $this->mastersecret);
        $template = $this->IGtNotificationTemplate("", $title, $content, "");   //透传消息模板
        //定义"ListMessage"信息体
        $message = new \IGtListMessage();
        $message->set_isOffline(true);//是否离线
        $message->set_offlineExpireTime(3600 * 12 * 1000);//离线时间
     ...
     * toList回归用户
     */
    public function pushMessageToAssignList($cid, $pushContent)
    {
        $igt = new \IGeTui($this->host, $this->appkey, $this->mastersecret);
        $template = $this->IGtNotificationTemplate('', $pushContent['title'], $pushContent['content'], '');   //通知栏消息
        //定义"ListMessage"信息体
        $message = new \IGtListMessage();
        $message->set_isOffline(true);//是否离线
        $message->set_offlineExpireTime(3600 * 12 * 1000);//离线时间
        ...
        foreach ($cid as $k => $v) {
            ...
            $tmp = (string)$v;
            $target->set_alias($tmp);     //设置别名
//            $target->set_clientId($tmp);
            $targetList[] = $target;
        }
        print_r($targetList) . PHP_EOL;
        Log::record("个推日志aaaaaaaaa-----" . json_encode($targetList), "info");
        $rep = $igt->pushMessageToList($contentId, $targetList, true);
        Log::record("个推日志-----" . json_encode($rep), "info");
        return $rep;
    }
...
//        $template->set_apnInfo($apn);
    ...
    //通知栏消息 (通知栏显示 点击启动应用)
    function IGtNotificationTemplate($trans_content, $title, $content, $logo)
    {
     ...
     * @throws \Exception
     */
    public function pushMessageToSingleBatch($pushList)
    {
        if (empty($pushList)) {
            return [];
        }
        ...
        Log::info("pushMessageToSingleBatch pushList" . json_encode($pushList));
        $igt = new \IGeTui($this->host, $this->appkey, $this->mastersecret);
        ...
        try {
            $rep = $batch->submit();
            Log::info("pushMessageToSingleBatch res submit" . json_encode($rep));
        } catch (\Exception $e) {
            Log::error(sprintf("pushMessageToSingleBatch error:%s errorstrace:%s", $e->getMessage(), $e->getTraceAsString()));
            $rep = $batch->retry();
            Log::info("pushMessageToSingleBatch res retry" . json_encode($rep));
        }
        return $rep;
    }
}