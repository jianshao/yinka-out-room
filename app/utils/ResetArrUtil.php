...
class ResetArrUtil
{
    ...
    public static function getInstance()
    {
        if (!isset(self::$instance)) {
            self::$instance = new ResetArrUtil();
        }
...
//     * @return array
//     */
    public function resetTagListData($data,
                                     $pid = 0,
                                     $fieldPri = 'cid',
                                     $fieldPid = 'pid',
                                     $level = 1)
    {
        ...
        foreach ($data as $k => $v) {
            if ($v[$fieldPid] == $pid) {
                $level++;
                $v['subsets'] = $this->resetTagListData($data, $v[$fieldPri], $fieldPri, $fieldPid, $level);
                $tempData[] = $v;
            }
        }
        return $tempData;
...
//     * @return array
//     */
    public function resetTagListDataSecond($data,
                                           $pid = 0,
                                           $fieldPri = 'cid',
                                           $fieldPid = 'pid',
                                           $level = 1)
    {
        ...
        foreach ($data as $k => $v) {
            if ($v[$fieldPid] == $pid) {
                $level++;
                $v['subsets'] = $this->getSubData($data, $v[$fieldPri], $fieldPri, $fieldPid, $level);
                $tempData[] = $v;
            }
        }
        return $tempData;
                                ...
                                $level = 1)
    {
        ...
        foreach ($data as $k => $v) {
            if ($v[$fieldPid] == $pid) {
                $tempData[] = $v;
                $tempData = array_merge($tempData, $this->getSubData($data, $v[$fieldPri], $fieldPri, $fieldPid, $level));
            }
        }
        return $tempData;
    }
    ...
    private function resetArrValue($arr)
    {
        $reset = [];
        if (!is_array($arr)) {
            return $arr;
        ...
        foreach ($arr as $kk => $vv) {
            foreach ($vv as $key => $value) {
                if (!is_array($value)) {
                    $reset[$kk][$key] = $this->resetArrValue($value);
                } else {
                    $reset[$kk][$key] = $this->resetArrValue($value);
                }
            }
    ...
    /**
     *
     * @info 根据传入的原始二维数据和id搜索需要的数组单元数据
     * @时间复杂度 O(log)
     * @param $originData array    //原始的二维数组item单元
     * @param $ids   mixed(int array)         //要搜索的item id
     * @param $fieldKey string  具体要搜索的字段field
     * @return mixed
     */
    public function searchItemForArr($originData, $ids, $fieldKey = 'id')
    {
        if (empty($originData)) {
            return array();
        }
        $uid = array_column($originData, $fieldKey);
        if (empty($uid)) {
            return array();
        }
        if (!is_array($ids)) {
            $key = array_search($ids, $uid);
            return $originData[$key];
        }
        $result = array();
        foreach ($ids as $v) {
            $key = array_search($v, $uid);
            $result[] = $originData[$key];
        }
        return $result;
    }
}
