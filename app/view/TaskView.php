...
use app\api\controller\v1\TaskController;
...
class TaskView
{
    ...
    // 任务列表展示
    public static function encodeContent($taskKind, $useName=0) {
        assert($taskKind instanceof TaskKind);
        ...
        for ($i = 0; $i < count($taskKind->reward); $i++){
            ...
            $dict = [
                ...
                'name' =>  $useName ? $asset->displayName : "+".$rewardItem->count,
                'num' =>  $rewardItem->count
            ];
    ...
    public static function encodeWeekCheckIn($task) {
        ...
        return [
            ...
            'retain' =>  strval($taskKind->count),
            'task_reward' =>  self::encodeReward($taskKind),
            'content' =>  self::encodeContent($taskKind, 1)[0],
		    "is_sign"=>  $task->hasReward()?1:0,
		    "task_type"=>  TaskView::$taskType['weekCheckIn'],
		    "task_status"=>  TaskView::$taskStatus['weekCheckIn'],
        ];
    }
    ...
    public static function encodeDaily($task) {
        ...
        return [
            ...
            'task_name' =>  $taskKind->name,
            'task_desc' =>  $taskKind->desc,
            'retain' =>  $taskKind->retain,
            'task_reward' =>  self::encodeReward($taskKind),
            'content' =>  self::encodeContent($taskKind),
            "is_finish"=>  $task->hasReward()?2:($task->isFinished()?1:0),
            "task_type"=>  TaskView::$taskType['daily'],
            "task_status"=>  TaskView::$taskStatus['daily'],
    ...
    public static function encodeNewer($task) {
        ...
        return [
            ...
            'content' =>  self::encodeContent($taskKind),
            "is_finish"=>  $task->hasReward()?2:($task->isFinished()?1:0),
            "task_type"=>  TaskView::$taskType['newer'],
            "task_status"=>  TaskView::$taskStatus['newer'],
            "toComplete"=> $taskKind->toComplete,
        ];
    }
