...
//华为推广上报service
class HuaweiPromoteService extends PromoteService
{
    ...
    public static function getInstance()
    {
        if (!isset(self::$instance)) {
            $object = new HuaweiPromoteService();
    ...
    public function onAndroidActivateEvent(AndroidActivateEvent $event)
    {
        ...
        $reportReFirst = HuaweiGalleryService::getInstance()->report($oaid, "1");
    ...
    public function onUserRegisterEvent(UserRegisterEvent $event)
    {
        ...
        $clientInfo = $event->clientInfo;
        $oaid = $clientInfo->oaid ?: "";
//        $reportReFirst = HuaweiGalleryService::getInstance()->report($oaid, "1", $event->userId);
        $reportReSecond = HuaweiGalleryService::getInstance()->report($oaid, "7", $event->userId);
        if (empty($reportReSecond)) {
            return false;
        }
        return true;
    }
...
//        过滤
        ...
        return HuaweiGalleryService::getInstance()->report($oaid, $eventType, $event->userId);
    }
