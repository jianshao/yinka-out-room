...
use app\domain\game\turntable\PoolStatics;
...
class TurntableUser
{
    ...
    public function __construct($userId, $turntableId) {
        $this->userId = $userId;
    ...
    public function entryPool($poolType, $poolId) {
        if ($this->curPoolType != $poolType || $this->curPoolId != $poolId) {
            if ($this->curPoolId != 0) {
                Log::info(sprintf('TurntableUser::entryPool changePool userId=%d old=%s:%d new=%s:%d',
    ...
    public function clearPoolStatics($poolId) {
        ...
        Log::info(sprintf('TurntableUser::clearPoolStatics userId=%d turntableId=%d poolId=%d', $this->userId, $this->turntableId, $poolId));
    }
